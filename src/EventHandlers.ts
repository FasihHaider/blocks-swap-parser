import { ERC20, Transfer, Token, onBlock, Block, Swap } from "generated";
import { getTokenInfo, type TokenInfo } from "./effects/getTokenInfo";
...
ERC20.Transfer.handler(async ({ event, context }) => {
  ...
  // If token doesn't exist, fetch and cache it
  if (!token) {
    const tokenInfo = await context.effect(getTokenInfo, tokenAddress) as TokenInfo;
    
    // Cache token with symbol and decimals in separate columns
    token = {
      id: tokenAddress,
      symbol: tokenInfo.symbol,      // Stored in 'symbol' column
      decimals: tokenInfo.decimals,  // Stored in 'decimals' column
    };
